  ...
  */
  public $global_options;
	...
	*/
	...
	/**
	* @var array of array Array of the public hierarchicals taxonomies slugs associated with an array
	*											of parent terms with at least one child term
	*/
	private $mtsw_default_parent_terms;
	...
	*/
	private $mtsw_default_children_terms;
	...
	*/
	private function __construct() {
		...
		// and linked to each other before 'load_term' is called
	...
	*/
	public function get_mtsw_default_parent_terms( $taxonomy_slug ) {
		if ( !isset( $this->mtsw_default_parent_terms[$taxonomy_slug] ) ) {
			if ( $taxonomy = get_taxonomy( $taxonomy_slug ) )
				$this->init_mtsw_terms_properties( $taxonomy_slug );
			else
				return array();
		}
	...
	*/
	private function init_mtsw_terms_properties( $taxonomy_slug ) {
		...
		if ( $taxonomy->public && $taxonomy->hierarchical ) {
			$children = _get_term_hierarchy( $taxonomy_slug );
			$mtsw_default_parent_terms_ids = array_keys( $children );
			$args = array(
				'hide_empty' => false,
				'include' => $mtsw_default_parent_terms_ids,
				'parent' => 0);
			...
			$this->mtsw_default_parent_terms[$taxonomy_slug] = array();
			$this->mtsw_default_children_terms[$taxonomy_slug] = array();
		}
	}
			...
			foreach ( $public_post_types as $public_post_type ) {
				...
				foreach ( $post_type_taxonomies as $post_type_taxonomy ) {
					if ( $post_type_taxonomy->public &&
							 ...
							 wp_count_terms( $post_type_taxonomy->name ) > 0 ) {
	...
	*/
	...
	function taxonomy_change_callback() {
		...
		if ( isset( $_POST['post_type'] ) &&
			...
			<input name="<?php echo $_POST['attr_pre_name']; ?>[excluded_children_term_ids]" type="hidden" value="" /> <?
		}
